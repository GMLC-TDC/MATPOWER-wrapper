...
to = find(gen_mix.Datetime == end_time+ hours(UTC_adjust));
...
figure,
subplot(2,1,1);
area(time_data, y_sim, 'LineStyle', 'none')
...
time_actual = gen_mix.Datetime(frm: to) - hours(UTC_adjust);


subplot(2,1,2);
area(time_actual, y_actual, 'LineStyle', 'none');
colororder(newcolors)
hold on
plot(time_data, total_load_t, 'k.', 'LineWidth', 1.5);
set(gca, 'YTickLabel',get(gca,'YTick'))
